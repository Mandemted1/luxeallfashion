...
import { customerOrders, findCustomer } from "@/lib/mock-customers";
import { orderTotalGhs } from "@/lib/mock-orders";
...
export default async function CustomerDetailPage(props: PageProps<"/customers/[id]">) {
  ...
  const orders = customerOrders(customer.email);
    ...
    <div>
      ...
      <div className="mt-6 border border-black/10 bg-white">
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.orderNumber} className="border-b border-black/5 last:border-b-0">
